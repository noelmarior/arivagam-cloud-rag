...
const UploadModal = ({ isOpen, onClose, folderId, onUploadComplete, onOptimisticUpload, sessionId }) => {
  ...
  const handleUpload = async () => {
    ...
    // Instead of locking the UI, immediately close and pass to dashboard
    onClose();
    setFiles([]);
    let allPromises = [];
    ...
    for (const file of files) {
      ...
      const uploadPromise = async () => {
        try {
          ...
          // Immediately display done to quickly close the modal
      ...
      // OPTIMISTIC UI: Instantly render the file as "processing" in the dashboard
      if (onOptimisticUpload) {
        onOptimisticUpload(file);
      }
    }

    // We intentionally DO NOT wait for Promise.all here because we want an instant close.
    // The background axios requests will still fire.
    ...
    >
      ...
      >
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {files.map((file, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700/50">
                  ...
                  <button onClick={() => setFiles(files.filter(f => f !== file))}><X className="w-4 h-4 text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400" /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="p-5 border-t border-gray-100 dark:border-gray-800/60 bg-gray-50 dark:bg-[#15171e] rounded-b-2xl flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition">Cancel</button>
          <button
